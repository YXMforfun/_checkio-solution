...
“Oh... Right...”
Stephen spoke up. “Well, this door back here has a keypad. Only thing is the brackets look pretty busted up. We could
try fixing it and then punching in the password?”
...
def checkio(expression):
    ...
    for i in expression:
        if i in close_open_map.values():
            stack.append(i)
        elif i in close_open_map.keys():
            if len(stack) == 0 or stack[-1] != close_open_map[i]:
                return False
            stack.pop()
    return len(stack) == 0
